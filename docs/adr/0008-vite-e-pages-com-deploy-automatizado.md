...
- Um workflow de CI verificará pull requests, `develop`, branches de release e `main`. O deploy estável publicará somente um estado verde de `main`, com permissões mínimas e concorrência que descarte publicações obsoletas.
- Alpha será distribuída por GitHub Prerelease e artifact da CI. Beta e RC também terão um preview identificado e isolado em `/next/` no mesmo GitHub Pages, sem substituir a aplicação estável na raiz nem compartilhar seu namespace de dados.
