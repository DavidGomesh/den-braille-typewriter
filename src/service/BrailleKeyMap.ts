...
export function keyCodeToKey(keyCode: number): Option<Key> {
    return pipe(keyCodeToKeyMap, getOption(keyCode))
}

export function keysToCell(keys: Set<Key>): Option<Cell> {
    return pipe(keysToCellMap, getOption(keys))
}
...
export function foldKeyState <A> (p: () => A, r: () => A): (s: KeyState) => A {
    return s => s == KeyState.PRESSED ? p() : r()
}

const keyCodeToKeyMap: Map<number, Key> = Map([
    [81, Key.ENTER], 
    [32, Key.SPACE], 
    [70, Key.DOT1], 
    [68, Key.DOT2], 
...
const keysToCellMap = Map([
    [Set([Key.ENTER]),                                                  Cell.ENTER],
